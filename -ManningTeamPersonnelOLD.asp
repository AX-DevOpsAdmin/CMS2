...
<form action="" method="POST" name="frmDetails">
    ...
    <!--#include file="Includes/hierarchyTeamDetails.inc"--> 
            ...
            <td>
                <table width="100%" border=0 cellpadding=0 cellspacing=0>
                    ...
                    <tr height=20>
                        ...
                        <td width=91% valign="middle" class=itemfont><font id=totalCount>&nbsp;</font>&nbsp;Posts - <u class=itemfontTip>(Tip: Columns "Rank", "Surname" and "Team" can be ordered by clicking on the column heading.)</U></td>
                    </tr>
                    <tr height=20>
                    	<td colspan="3">&nbsp;</td>
                    </tr>
            ...
            <td>
                <table border=0 cellpadding=0 cellspacing=0 >
                    <tr>
                       ...
                       <td class=toolbar valign="middle" >Personnel Present (<font id=presentCount>&nbsp;</font>): <%=thisDate%></td>
                    </tr>  
                </table>
            </td>
        </tr>
		...
		<tr height=35%>
			<td valign=top>
                <table width=965px border=0 cellpadding=0 cellspacing=0>
				...
				<Div class="ScrollingAreaTeams">
					<table width=965px border=0 cellpadding=0 cellspacing=0 style="table-layout:fixed;">
						<%do while not rsRecSet.eof%>
                            ...
                            <tr id="<%=rsRecSet("postID")%>" <%if int(rsRecSet("postID"))=int(request("fromSearch")) then%><% tempPostID = rsRecSet("postID") %><% tempstaffPostID=rsRecSet("staffPostID") %><% tempServiceno=rsRecSet("serviceno") %><%end if%> class=itemfont height=20 style="<%if counter=0 then%>background-color:<%=color1%>;<%if strManager=1 or cint(session("Administrator"))=1 then%>cursor:hand;<%end if%><%else%>background-color:<%=color2%>;<%if strManager=1 or cint(session("Administrator"))=1 then%>cursor:hand;<%end if%><%end if%>" <%if strManager=1 or cint(session("Administrator"))=1 then%>onClick="postItemOnclick(document.getElementById('<%=rsRecSet("postID")%>'),'<%=rsRecSet("postID")%>','<%=rsRecSet("staffPostID")%>','<%=rsRecSet("serviceno")%>','<%=rsRecSet("Ghost")%>');"<%end if%>>
                                <td width=8px>&nbsp;</td>
                                ...
                                <td width=35px align="center" <% if rsRecSet("Ghost") = 0 then %><%if rsRecSet("serviceno")<>"" then%><%if strManager=1 then%>onclick="showMenu('<%=rsRecSet("description")%>');"<%end if%><%end if%><% end if %>><img src="<% if rsRecSet("Ghost") = 0 then %>Images/itevent.gif<% else %>Images/itevent_gray.gif<% end if %>"></td>
                            </tr>
                            <tr>
                                <td colspan=14 class=titlearealine  height=1></td> 
                            </tr>
	...
	<Div id="TaskList" name="TaskList" style="visibility:hidden;position:absolute;top:100px;left:798px;background-color:#ffd777;">
		<table width=130px border=0 cellpadding=0 cellspacing=0>
			<tr>
				<td colspan=2 height=4px></td> 
			</tr>
			<tr class="itemfont whiteOnBlack" height=20px>
				<td align="center" colspan=2 id="postNameForTasking"></td>
			</tr>
			<tr>
				<td colspan=2 height=4px></td> 
			</tr>
			...
			<%do while not rsTaskTypes.eof%>
                ...
                <tr style="cursor:hand;" id="<%=rsTaskTypes("Description")%>" name=id="<%=rsTaskTypes("Description")%>" class=itemfont height=20px onMouseOver="taskItemOver(this);" onMouseOut="taskItemOut(this);">
                    <td width=4px></td><td onclick="gotoTask('<%=rsTaskTypes("ttID")%>','<%=rsTaskTypes("Description")%>')"><%=rsTaskTypes("Description")%></td>
                </tr>
...
window.parent.frmDetails.thisIframe.value="ManningTeamPersonnel.asp"
window.parent.crumbTrail.innerHTML="<A title='' href='index.asp' class=itemfontlinksmall >Home</A> > <font class='youAreHere' >Personnel</font>"
