...
' ------------------------------------------------------------------------------
	Public Sub SaveAs(ByRef pstrFileName)
		...
		' Turn on error checking as write will fail if user doesn't have Write permission.
		On Error Resume Next
			Call lobjStream.SaveToFile(pstrFileName, adSaveCreateOverWrite)
			Select Case Err.Number
				Case 0
					'Do nothing. This is a successful upload!
				Case 3004
					response.Write("You do not have permission to write to this folder.<br>Please contact your SIM for assistance.")
					...
					Exit Sub
				Case Else
					response.Write("An unexpected error has occurred.<br>Please contact your SIM with the following information:<br>Error number: " & Err.Number & "<br> Description: " & Err.Description & "<br>ASP Page: " & request.ServerVariables("HTTP_REFERER"))
					' Close the stream object
					Call lobjStream.Close()
					' Release objects
					Set lobjStream = Nothing
