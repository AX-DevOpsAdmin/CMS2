...
<table border=1 cellpadding=0 cellspacing=0 width=80% align="center">
    ...
    <tr>
        <td height=20 class="itemfont">&nbsp;<%=rsAuths("assignno")%></td>
        <td height=20 class="itemfont">&nbsp;<%=rsAuths("serviceno")%></td>
        <td height=20 class="itemfont">&nbsp;<%=strName%></td>
        <td height=20 class="itemfont">&nbsp;<%=rsAuths("wg")%></td>
        <td height=20 class="itemfont">&nbsp;<%=rsAuths("sqn")%></td>
        <td height=20 class="itemfont">&nbsp;<%=rsAuths("post")%></td>
    </tr>
              
    <% intCount = 1 %>
    <% if not rsAuths.eof then %>
        <% do while not rsAuths.eof %>
            ...
            <% end if %>
            <tr>
                <td height=20 class="itemfont">&nbsp;<%=rsAuths("expiry")%></td>
                <td height=20 class="itemfont" >&nbsp;<%=rsAuths("description")%></td>
                <td height=20 class="itemfont" colspan="2">&nbsp;<%=rsAuths("longdesc")%></td>
                <td height=20 class="itemfont">&nbsp;<%=rsAuths("assessor")%></td>
                 <td height=20 class="itemfont">&nbsp;<%=rsAuths("approver")%></td>
            </tr>
            <% intCount = intCount + 1 %>
            <% rsAuths.movenext %>
        <% loop %>
