...
<body>
<form action="RemoveMilSkills.asp" method="post" name="frmDetails">
	...
	<input type="hidden" name="ReturnTo" id="ReturnTo" value="HierarchyPersMSRemove.asp"/>
                            ...
                            <table border="0" cellpadding="0" cellspacing="0" width="100%">
                                <tr>
                                    <td>
                                        <table border=0 cellpadding=0 cellspacing=0 width=100%>
                                            <tr class="SectionHeader toolbar">
                                                <td height="25px" align="left">Military Skills Held</td>
                                                <td height="25px">&nbsp;</td>
                                                <td height="25px" align="left">Military Skills to Remove</td>
											</tr>
                                            <tr>
                                                ...
                                                <td class="ColorBackground">
													<select name="lstAttached" size="10" multiple class="pickbox" style="width:180px;" id="lstAttached" onChange="remAttached()"> 
														<% if not rsRecSet.bof and not rsRecSet.eof then %>
															<% do while not rsRecSet.eof %>
																<option value=<%= rsRecSet("StaffMSid") %>><%= rsRecSet("Description") %></option>
																<% rsRecSet.movenext() %>
															<% loop %>
														<% end if %>
													</select>
...
function saveNew()
{
	...
	{
		errMsg += "Select Military Skills Held";
