...
<body>
<form action="RemoveFitness.asp" method="post" name="frmDetails">
	...
	<input type="hidden" name="ReturnTo" id="ReturnTo" value="hierarchyPersFitnessRemove.asp"/>
                            ...
                            <table border="0" cellpadding="0" cellspacing="0" width="100%">
                                <tr>
                                    <td>
                                        <table border=0 cellpadding=0 cellspacing=0 width=100%>
                                            <tr class="SectionHeader toolbar">
                                                <td height="25px" align="left">Fitness Currencies Held</td>
                                                <td height="25px">&nbsp;</td>
                                                <td height="25px" align="left">Fitness Currencies to Remove</td>
											</tr>
                                            <tr>
                                                ...
                                                <td class="ColorBackground">
													<select name="lstAttached" size="10" multiple class="pickbox" style="width:180px;" id="lstAttached" onChange="remAttached()"> 
														<% if not rsRecSet.bof and not rsRecSet.eof then %>
															<% do while not rsRecSet.eof %>
																<option value=<%= rsRecSet("StaffFitnessid") %>><%= rsRecSet("Description") %></option>
																<% rsRecSet.movenext() %>
															<% loop %>
														<% end if %>
													</select>
...
function saveNew()
{
	...
	{
		errMsg += "Select Fitness Types Held";
