...
' They have been authorised to Assess/Approve Auths so check if any are outstanding
...
if session("serviceNo") > "" then
	for x = 1 to objCmd.parameters.count
		objCmd.parameters.delete(0)
	next

	ObjCmd.CommandText = "spGetConfirmAuthCount"
	ObjCmd.Parameters.Append ObjCmd.CreateParameter("@staffID", 3,1,0, session("staffID"))
	ObjCmd.Parameters.Append ObjCmd.CreateParameter("@rtn", 3,2,0)
	ObjCmd.execute
	
	conCount = ObjCmd.Parameters("@rtn")
end if
...
<html>
  ...
  <body style="margin-left:0;" onLoad="chk();  <%if session("authorisor") > 0 and persSearch = 0 then%> loadApproved(<%=appCount%>,<%=histCount%>, <%=session("staffID")%>); <%end if%>; loadConfirmation(<%=conCount%>,<%=session("staffID")%>);" onResize="chk();">
        ...
        <table cellspacing="0" cellPadding="0" width="100%" border="0">
            <tr>
                <td>
                    ...
                    <table cellSpacing="0" cellPadding="0" width="100%" border="0" height="<%=myHeight1%>" >
                                                ...
                                                mLeft = 10 * rsHrc("hrclevel")
                                                 ...
                                                 elseif rsHrc("hrcChildren")= true then  ' This has children but is not on the open branch
                                                    ...
                                                    if rsHrc("hrclevel")> 1  then
                                                       if rsHrc("hrclevel")< rsHrc("lastlevel") and (rsHrc("hrcparentid") = lvl1id or  rsHrc("hrcparentid") = parentid or rsHrc("hrclevel")=openlvl )  then
                                                          imgclass="imgopen"	
                                                        else	
                                                          imgclass="imgclosed"
                                                        end if							 
    ...
    <div id="light-lims" style="display:none;"  class="white_content">
       ...
       <div class="lightboxcontent" align="center">
           <textarea name="limits" rows="10" class="itemfont" id="limits" style="width:90%; height:220px; resize:none;" readonly></textarea>
       </div> 
...
function passLevels(xoom, hrcID, persSearch)
{
   // alert("URL is " + xoom + " * " + persSearch);
	...
	document.getElementById("teamIframe").src = xoom+"&initialDate="+thisDate+"&thisDate="+thisDate + "&fromSearch=" + fromSearch + "&allTeams=" + allTeams +"&persSearch="+persSearch;
...
//	alert(viewportHeight)
	...
	side_div.style.height = (height - elemPosition(side_div).top) + "px";
...
function loadConfirmation(conCount, staffID){
	
	//alert("Auth count is " + appCount + " History count is " + histCount);
	if(parseInt(conCount) > 0){
		var s = ""
		...
		confirmCount.innerHTML = conCount;	
		confirmCount.onmouseover = function(){confirmCount.title='You have '+conCount+' Authorisation' + s + ' awaiting to be accepted.';}
		//notifCount.onclick = function(){window.location = 'cms_hierarchy3.asp?persSearch=2'}
	}
	else if(parseInt(histCount) > 0){
		var notification = document.getElementById('notification');
