...
<form  action="AddCycleStages.asp?strAction=<%=strAction%>" method="POST" name="frmDetails">
  ...
  <table  height="100%" cellspacing=0 cellPadding=0 width=100% border=0>
        ...
        <!--#include file="Includes/Header.inc"--> 
  		 	<table cellSpacing=0 cellPadding=0 width=100% border=0 >
                    <tr style="font-size:10pt;" height=26px>
                        ...
                        <td class="staticdatamenu"><font class="youAreHere">CMS Admin Menu / <strong>Groups</strong></font></td>
                    </tr>
				  ...
				  <tr>
					...
					<table width=100% border=0 cellpadding=0 cellspacing=0>
					  ...
					  <tr class=columnheading height=22>
                        ...
                        <td valign="middle" width="3%">&nbsp;</td>
                        <td valign="middle" width="30%">Cycle Stages to Add</td>
                        <td valign="middle" width="21%">&nbsp;</td>
                      </tr>
                      <tr>
                        <td valign="middle" width="2%">&nbsp;</td>
                        <td valign="top" width="13%" class="itemfont">
						    <table width="100%">
							  <tr>
                                <td>&nbsp;</td>
                              </tr>
						...
						<td valign="middle" width="3%">&nbsp;</td>
                        <td valign="middle" width="30%" class="itemfont">
                          <select name="lstAttached" size="10" multiple class="pickbox" id="lstAttached" onChange="remAttached()">
                            <%do while not rsCyAdd.eof%>
                            <option value = "<%= rsCyAdd("cysID") %>" ><%= rsCyAdd("description") %></option>
                            <% rsCyAdd.movenext
			                loop%>
                          </select>
                        </td>
    ...
    <input id="hiddenChange" name="hiddenChange" type="hidden" value="">
  <input id="currStages" name="currStages" type="hidden" value="<%=strCurrStages%>">
  <input id="newattached" name="newattached" type="hidden" value="">
  <input type="hidden" id="ReturnTo" name="ReturnTo" value="AdminCycleDetail.asp"> 
</form>
<%
rsRecSet.close
...
function saveNew(){
    ...
    if (document.frmDetails.hiddenChange.value == "") {
		  errMsg += "There are no Stages to Add to this Cycle\n"
	...
	if (document.frmDetails.pickAttached.options.length != 0) {
	     ...
	     /* now build hidden value with list of Locations to submit so the 
	        program writelocations can update database - meed the ',' at the end of the string for the stored procedure
			to pick all the elements up */
	     if (currstages.length != 0) {
		   newattached = currstages + document.frmDetails.pickAttached[0].value  + ","; }
		 else  {
   ...
   if(!errMsg=="") {
	...
	document.frmDetails.hiddenChange.value = "";
	//alert("Just Added " + document.frmDetails.newattached.value);   
	document.frmDetails.submit();  
}
