...
<body>
<form action="RemoveStaffAuths.asp" method="post" name="frmDetails">
	...
	<input type="hidden" name="ReturnTo" id="ReturnTo" value="HierarchyPersAuthorise.asp"/>
                                                ...
                                                <td class="ColorBackground">
													<select name="lstAttached" size="10" multiple class="pickbox" style="width:180px;" id="lstAttached" onChange="remAttached()"> 
														<% if not rsRecSet.bof and not rsRecSet.eof then %>
															<% do while not rsRecSet.eof %>
																<option value=<%= rsRecSet("staID") %>><%= rsRecSet("authCode") %></option>
																<% rsRecSet.movenext() %>
															<% loop %>
														<% end if %>
													</select>
...
function saveNew()
{
	...
	{
		errMsg += "Select Authorisations Held";
