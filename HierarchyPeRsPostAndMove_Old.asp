		...
		<tr>
			<td>
				<table border=0 cellpadding=0 cellspacing=0 width=100%>
					<tr>
						<td>
							<table width=468px border=0 cellpadding=0 cellspacing=0>
								<tr height=24px>
						...
						<td>
							<table width=468px border=0 cellpadding=0 cellspacing=0>
								<tr height=24px>
									<td colspan=5 class=toolbar align="center"><u>Movements</u></td>
								</tr>
								<tr class="calenderLightOrange columnheading"   height=20>
                                    ...
                                    <td align="center" ><% if strManager = 1 then%>Untask<% else %>&nbsp;<%end if%></td>
								</tr>
                                <tr>
                                    <td colspan=5 class=titlearealine  height=1></td> 
                                </tr>
								...
								<table width=468px border=0 cellpadding=0 cellspacing=0>
									<% do while not rsRecSet.eof %>
										<tr id="<%= rsRecSet("taskStaffID") %>" class=itemfont height=20 <% if counter = 0 then %>style="background-color:<%= color1 %>;"<% else %>style="background-color:<%= color2 %>;cursor:hand;"<% end if %>>
											...
											<td align="center"><% if strManager = 1 then %><input name="taskStaffID" id="taskStaffID" value ="<%= rsRecSet("taskStaffID") %>" type=checkbox><% else %>&nbsp;<% end if %></td>					
										</tr>
                                        <tr>
                                            <td colspan=5 class=titlearealine  height=1></td> 
                                        </tr>
