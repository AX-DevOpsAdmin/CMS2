...
' now get the AuthLevels available for staff at current RANK
strCommand = "spGetStaffAuthLevels"
objCmd.CommandText = strCommand

set rsLvl = objCmd.Execute	'Execute CommandText when using "ADODB.Command" object'

'response.write (strCommand & " * " & request("staffID"))
                                                ...
                                                <tr class="columnheading" height="22px">
                                                    ...
                                                    <!--<div id="authclass" style="float:left"> 
                                                        ...
                                                        <select name="authclassID" id="authclassID" class="itemfont" style="width:50px">
                                                            ...
                                                            <option value=1 <% 'if rsRecSet("authclass") = 1 then %> selected <%' end if %>>K</option>
                                                            <option value=2 <%' if rsRecSet("authclass") = 2 then %> selected <%' end if %>>J</option>
                                                            <%'end if %> 
                                                    ...
                                                    <select class="itemfont" name="lvlID" id="lvlID" style="width:120px">
                                                       <option value=0>Select..</option>
                                                       <%do while not rsLvl.eof%>
                                                            <option value=<%=rsLvl("lvlID")%> ><%=rsLvl("authlevel")%></option>
                                                            <%rsLvl.Movenext %>
                                                       <% loop%>
                                                        <!-- They are either Auth Admins or Level K Authorisors -->
                                                        <% 'if session("authAdmin") = 1 or strAuth=1 then %>
                                                           <!--<option value=99 >Special K Auth</option>-->
                                                        <% 'end if %>
                                                    </select>
...
{	
	   ...
	   // 
		...
		if(document.frmDetails.lvlID.value == 0)
		{
