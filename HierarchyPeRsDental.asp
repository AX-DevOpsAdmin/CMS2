...
objCmd.CommandText = "spPersDentalObtained"	'Name of Stored Procedure
...
<form action="" method="POST" name="frmDetails">
	<input name="staffID" name="staffID"  type="hidden" value="<%=request("StaffID")%>">
			...
			<td>
				<% if strManager = 1 then %>
					<table border=0 cellpadding=0 cellspacing=0 >
						<tr>
							<td height="25px" class=toolbar width=8><td width=20><a class=itemfontlink  href="hierarchyPersDentalAdd.asp?staffID=<%=request("staffID")%>&thisDate=<%=request("thisDate")%>"><img class="imagelink" src="images/editgrid.gif"></A></td>
							<td height="25px" class=toolbar valign="middle">Add Dental Type</td>
							<td height="25px" class=titleseparator valign="middle" width=14 align="center">|</td>
							<td height="25px" width=20><a class=itemfontlink href="hierarchyPersDentalRemove.asp?staffID=<%=request("staffID")%>&thisDate=<%=request("thisDate")%>"><img class="imagelink" src="images/editgrid.gif"></A></td>
							<td height="25px" class=toolbar valign="middle">Remove Dental Type</td>
						</tr>
					</table>
				<% end if %>
			</td>
						...
						<td width="50%" align="left" height="22px" colspan=5>
    						<table border=0 cellpadding=0 cellspacing=0 width=100%>
    							<tr class="SectionHeader toolbar">
                                    <td width="32%" align="left" height="25px">Summary of Dental Types (One Required)</td>
								...
								<% if rsQualificationDetails.recordcount > 0 then %>
									<% do while not rsQualificationDetails.eof %>
                                    	<% strDental = rsQualificationDetails("description") %>
                                        ...
                                        <tr>
                                        	<% if rsQualificationDetails("staffID") <> "" then %>
												<td class=toolbar><% if strManager = 1 then %><a href="javascript:DisplayForm('<%=formatdatetime(strValidFrom,2)%>','<%=strCompetent%>','<%=rsQualificationDetails("StaffDentalID")%>','<%=strDental%>');" onclick="DisplayForm" class=ItemLink><%=strDental%></A><% else %><%= strDental %><% end if %></td>
...
<form action="ManningPersSingleDentalUpdate.asp" method="post" name="popupDetails">
	<input type=hidden name=staffID value=<%=request("staffID")%>>
	<input type="hidden" name="ReturnTo" id="ReturnTo" value="hierarchyPersDental.asp">
	<input Type="Hidden" name="staffDentalID" id="staffDentalID">
	  ...
	  <table border=0 cellpadding=0 cellspacing=0 width=100%>
		  ...
		  <% else %>
			  <tr class=columnheading height=22>
				  ...
				  <td valign="middle" width=68% class=itemfont><input type=hidden name=Competent value=N></td>
			  </tr>
		  <% end if %>
		  <tr>
...
function DisplayForm(ValidDate,Competent,staffDentalID,Description)
{
	document.popupDetails.staffDentalID.value = staffDentalID;
