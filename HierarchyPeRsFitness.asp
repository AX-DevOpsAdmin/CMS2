...
objCmd.CommandText = "spPeRsFitnessObtained"	'Name of Stored Procedure
			...
			<td>
				<% if strManager = 1 then %>
					<table border=0 cellpadding=0 cellspacing=0 >
						<tr>
							<td height="25px" class=toolbar width=8>
                            <td height="25px" width=20><a class=itemfontlink href="HierarchyPersFitnessAdd.asp?staffID=<%=request("staffID")%>&thisDate=<%=request("thisDate")%>"><img class="imagelink" src="images/editgrid.gif"></A></td>
							<td height="25px" class=toolbar valign="middle">Add Fitness</td>
							<td height="25px" class=titleseparator valign="middle" width=14 align="center">|</td>
							<td height="25px" width=20><a class=itemfontlink href="HierarchyPersFitnessRemove.asp?staffID=<%=request("staffID")%>&thisDate=<%=request("thisDate")%>"><img class="imagelink" src="images/editgrid.gif"></A></td>
							<td height="25px" class=toolbar valign="middle">Remove Fitness</td>
						</tr>
					</table>
				<% end if %>
			</td>
            ...
            <td>
                <table border=0 cellpadding=0 cellspacing=0 width=100%>
					...
					<tr class=columnheading> 
						...
						<td align="left" width="25%" height="22px" class=itemfont><% if rsRecSet("remedial") = true then %><img src="images/yes.gif"><% else %><img src="images/no.gif"><% end if %></td>
						<td align="left" width="13%" height="22px">Exempt:</td>
						<td align="left" width="25%" height="22px" class=itemfont><% if rsRecSet("exempt") = true then %><img src="images/yes.gif"><% else %><img src="images/no.gif"><% end if %></td>
						<td align="left" width="22%" height="22px">&nbsp;</td>
					</tr>
					<tr class=columnheading> 
						<td align="left" width="2%" height="22px">&nbsp;</td>
						<td align="left" width="13%" height="22px">Expiry Date:</td>
						<td align="left" width="25%" height="22px" class=itemfont><% if rsRecSet("remedial") = true and rsRecSet("expiryDate") <> "" then %><% if strManager = 1 then %><a href="javascript:DisplayForm1('<%=rsRecSet("remedial")%>','<%=rsRecSet("exempt")%>')" onclick="DisplayForm1" class="ItemLink"><%= rsRecSet("expiryDate") %></a><% else %><%= rsRecSet("expiryDate") %><% end if %><% else %>-<% end if %></td>
						<td align="left" width="13%" height="22px">Expiry Date:</td>
						<td align="left" width="25%" height="22px" class=itemfont><% if rsRecSet("exempt") = true and rsRecSet("expiryDate") <> "" then %><% if strManager = 1 then %><a href="javascript:DisplayForm1('<%=rsRecSet("remedial")%>','<%=rsRecSet("exempt")%>')" onclick="DisplayForm1" class="ItemLink"><%= rsRecSet("expiryDate") %></a><% else %><%= rsRecSet("expiryDate") %><% end if %><% else %>-<% end if %></td>
						<td align="left" width="22%" height="22px">&nbsp;</td>
					</tr>
                    <tr>
                        ...
                        <td width="50%" align="left" height="22px" colspan=5>
                            <table border=0 cellpadding=0 cellspacing=0 width=100%>
                                <tr class="SectionHeader toolbar">
                                    <td width="32%" align="left" height="25px">Summary of Fitness Types</td>
								...
								<% if rsQualificationDetails.recordcount > 0 then %>
									<% do while not rsQualificationDetails.eof %>
                                    	<% strFitness = rsQualificationDetails("description") %>
                                        ...
                                        <tr>
                                        	<% if rsQualificationDetails("staffID") <> "" then %>
												<td align="left" height="22px" class=toolbar><% if strManager = 1 then %><a href="javascript:DisplayForm('<%=formatdatetime(strValidFrom,2)%>','<%=strCompetent%>','<%=rsQualificationDetails("StaffFitnessID")%>','<%=strFitness%>');" onclick="DisplayForm" class=ItemLink><%=strFitness%></A><% else %><%=strFitness%><% end if %></td>
											...
											<% else %>
                                            	<td class=toolbar><%=strFitness%></td>
...
<form action="" method="post" name="popupDetails1">
	<input type=hidden name="staffID" id="staffID" value=<%=request("staffID")%>>
	<input type=hidden name="newexpirydateattached" id="newexpirydateattached" value="">    
	<input type="hidden" name="ReturnTo" id="ReturnTo" value="HierarchyPersFitness.asp">
        ...
        <table border=0 cellpadding=0 cellspacing=0 width=100%>
            ...
            <tr class=columnheading>
                ...
                <td align="left" height=22px width=62% class=itemfont> 
                    <input name="expirydate" type="text" id="expirydate" class=" itemfontEdit inputboxEdit" style="Width:85px;"  value ="<%=rsRecSet("expiryDate")%>" readonly onclick="calSet(this)">&nbsp;
            ...
            <tr>
                <td height=22px align="center" colspan=2><Input CLASS="StandardButton" Type=Button style="width:60px;" Value=Cancel onclick="PopUpwindow2.style.visibility = 'hidden';"><Input CLASS="StandardButton" Type=Button style="width:60px;" Value=OK onclick="SaveNewExpiry();"></td>
            </tr>
            <tr>
...
<form action="ManningPersSingleFitnessUpdate.asp" method="post" name="popupDetails">
	<input type=hidden name=staffID value=<%=request("staffID")%>>
	<input type="hidden" name="ReturnTo" id="ReturnTo" value="HierarchyPersFitness.asp">
	<input Type="Hidden" name="StaffFitnessID" id="StaffFitnessID">
	
  <div id="PopUpwindow1" class="PopUpWindow">
	  <table border=0 cellpadding=0 cellspacing=0 width=100%>
		  <tr>
			  <td colspan=3 height=22 align="center" class=MenuStyleParent><u>Confirm Fitness Details</u></td>
		  </tr>
          <tr>
		  ...
		  <% end if %>
		  <tr class=columnheading height=22>
			  <td align="center" colspan=3><Input CLASS="StandardButton" Type=Button style="width:60px;" Value=Cancel onclick="PopUpwindow1.style.visibility = 'hidden';"><Input CLASS="StandardButton" Type=Button style="width:60px;" Value=OK onclick="popupDetails.submit();"></td>
		  </tr>
		  <tr>
...
function DisplayForm1(remedial,exempt)
{
	document.getElementById('PopUpwindow1').style.visibility = "hidden";
	var PopUpwindow2 = document.getElementById('PopUpwindow2');
	PopUpwindow2.style.visibility = "Visible";
	
	if(remedial == 'True')
	{
		var title = "Confirm Remedial Details";
		document.forms.popupDetails1.action = "AddRemedial.asp";
	}
	else
	{
		var title = "Confirm Exempt Details";
		document.forms.popupDetails1.action = "AddExempt.asp";
	}
	
	document.getElementById('Title').innerHTML = title;
	document.getElementById('Title').style.textDecoration = "underline";
	document.getElementById('PopUpwindow2').style.visibility = "Visible";	
}

function SaveNewExpiry()
{
	
	document.popupDetails1.newexpirydateattached.value = document.getElementById('expirydate').value;
	document.popupDetails1.submit();
}

function DisplayForm(ValidDate,Competent,staffFitnessID,Description)
{
	document.popupDetails.StaffFitnessID.value = staffFitnessID;
